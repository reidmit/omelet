...
var runtime = omelet$runtime = require('./runtime.js')
...
Compiler.prototype = {
    ...
    visit: function(node) {
        ...
        switch (node.kind) {
                ...
                for (i = 0; i < node.entries.length; i++) {
                    ...
                    if (kind === 'Block') {
                        ...
                        for (j = 0; j < node.entries[i].parameters.length; j++) {
                            var param = node.entries[i].parameters[j].value
            ...
            case 'If':
                this.stopStr()
                if (node.negated) {
                    this.write('if(omelet$runtime.falsy(')
                } else {
                    this.write('if(omelet$runtime.truthy(')
                }
                if (node.filters !== null) {
                    this.write(this.applyFn + '(')
                ...
                if (node.elifCases.length > 0) {
                    for (i = 0; i < node.elifCases.length; i++) {
                        ...
                        if (nodey.negated) {
                            this.write('else if(omelet$runtime.falsy(')
                        } else {
                            this.write('else if(omelet$runtime.truthy(')
                        }
    ...
    compile: function() {
        ...
        if (this.minify) {
            fn = UglifyJS.minify(fn.toString(), {fromString: true}).code
        }
        return fn
    }
}
