...
function Compiler(ast, options) {
    ...
    this.inBlockDefStk = []
    this.inStr = false
    this.inSingleLineTag = false
    this.throwOnUndefinedVariable = true
...
Compiler.prototype = {
    ...
    getFreeVariable() {
        this.variableCount++;
        return "om$var_" + this.variableCount;
    },
    ...
    visit: function(node) {
        ...
        switch (node.kind) {
            ...
            case 'For':
                this.stopStr()
                var indexVariable = this.getFreeVariable()
                this.write('for (var ' + indexVariable + '= 0;' + indexVariable + ' <')
                ...
                if (node.filters !== null) {
                    this.write('.length; ' + indexVariable + '++){\n' + this.scope + '.open();\n' +
                    ...
                    this.visit(node.filters)
                    this.write(',"' + this.fileName + '",' +
                                this.source + ',' + node.filters.line + ')[' + indexVariable + ']);\n')
                } else {
                    this.write('.length;' + indexVariable + '++){\n' +
                                this.scope + '.open();\n' +
                                this.scope + '.add("' + node.iterator.value + '",')
                    this.visit(node.data)
                    this.write('[' + indexVariable + ']);\n')
                }
                this.startStr()
                this.visit(node.body)
                this.writeIndent()
            ...
            case 'Identifier':
                ...
                this.write(this.scope + '.find("' + node.value + '", ' + this.throwOnUndefinedVariable)
            ...
            case 'If':
                ...
                if (node.filters !== null) {
                    this.write(this.applyFn + '(')
                    // Since this If statement may be checking for the existence
                    // of this variable, we don't throw if it is not found here
                    this.throwOnUndefinedVariable = false
            ...
            case 'Tag':
                ...
                try {
                    found = this.tagScope.find(node.name, true)
                } catch (e) {
                    found = false
                }
                ...
                if (node.tagStyle !== 'interpolated'
                    && !this.inSingleLineTag
                    && node.line !== 0) {
                    this.writeIndent()
                }
                ...
                if (elements.void[node.name] && node.inner.contents.length === 0) {
                    this.write('/>')
                } else {
                    this.write('>')
                    ...
                    if (node.line === node.lineEnd) {
                        this.inSingleLineTag = true
                    }
                    ...
                    if (node.line < node.lineEnd && node.tagStyle !== 'interpolated') {
                        this.writeIndent()
                    }
                    this.write('</' + node.name + '>')
                }
    ...
    compile: function() {
        ...
        var fnBody = '{return ' +
            'function ' + this.templateFn + '(' + this.ctx + '){\n' +
                'var ' + this.out + '="",' + this.getFreeVariable() + '=0,' +
                this.buildRuntime() + ',' +
                this.ctx + '=' + this.ctx + '||{},' +
