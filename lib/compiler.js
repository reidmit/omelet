...
Compiler.prototype.visit = function(node) {
    ...
    case 'Definition':
        ...
        if (node.parameters.length === 0) {
            ...
            for (i = 0; i < node.parameters.length; i++) {
                params.push(node.parameters[i].value)
                this.write(node.parameters[i].value)
            ...
            for (i = 0; i < params.length; i++) {
                this.write(this.scope + '.add("' + params[i] + '",' + params[i] + ');')
            }
            this.write('var ret=')
            kind = node.body.kind
            if (kind === 'String' || kind === 'Block') {
    ...
    case 'If':
        ...
        this.write('if(')
        if (node.negated) {
            this.write('runtime.falsy(')
        } else {
            this.write('runtime.truthy(')
        }
        if (node.filters !== null) {
            this.write(this.applyFn + '(')
    ...
    case 'Extend':
    case 'Path':
        throw Error(node.kind + ' nodes should have been resolved by linker.')
    default:
...
Compiler.prototype.compile = function() {
    ...
    var fn = new Function(this.ctx, 'runtime', ('var ' + this.out + '="",' +
        this.idx + '=0,' +
        this.ctx + '=' + this.ctx + '||{},' + this.scope + '=new runtime.Scope();' +
        this.scope + '.addAll(' + this.ctx + ');' +
        this.out + '+="' + this.buffer.join('') + '";return out;').replace(/\n/g,'\\n'))
    return fn
}
