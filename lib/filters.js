...
function eq(input, other) {
    var t1 = runtime.typeOf(input),
        t2 = runtime.typeOf(other),
        i
    if (t1 !== t2) {
        return false
    }
...
function sort_by(input, prop) {
    ...
    function compareBy(a, b) {
        ...
        var valA, valB
        if (runtime.typeOf(dateA) === 'Date'
            && !isNaN(dateA.getTime()) &&
            runtime.typeOf(dateB) === 'Date'
            && !isNaN(dateB.getTime())) {
            valA = dateA
            valB = dateB
        } else {
...
function date_format(input, formatStr) {
    var d = new Date(input)
    if (isNaN(d.valueOf())) {
        throw Error('Invalid input to \'date_format\' filter.' +
            ' Could not convert \'' + input.toString() + '\' to a Date object.')
    }

    var s = formatStr.toString()
    ...
    function toDayName(n, fullName) {
        switch (n) {
            case 0: return fullName ? 'Sunday' : 'Sun'
            case 1: return fullName ? 'Monday' : 'Mon'
            case 2: return fullName ? 'Tuesday' : 'Tues'
            case 3: return fullName ? 'Wednesday' : 'Wed'
            case 4: return fullName ? 'Thursday' : 'Thurs'
            case 5: return fullName ? 'Friday' : 'Fri'
            case 6: return fullName ? 'Saturday' : 'Sat'
            default: return ''
        }
    }

    function toMonthName(n, fullName) {
        switch (n) {
            case 0: return fullName ? 'January' : 'Jan'
            case 1: return fullName ? 'February' : 'Feb'
            case 2: return fullName ? 'March' : 'Mar'
            case 3: return fullName ? 'April' : 'Apr'
            case 4: return fullName ? 'May' : 'May'
            case 5: return fullName ? 'June' : 'Jun'
            case 6: return fullName ? 'July' : 'Jul'
            case 7: return fullName ? 'August' : 'Aug'
            case 8: return fullName ? 'September' : 'Sept'
            case 9: return fullName ? 'October' : 'Oct'
            case 10: return fullName ? 'November' : 'Nov'
            case 11: return fullName ? 'December' : 'Dec'
            default: return ''
        }
    }

    // Days
    s = replaceAll(s, '%dd', pad(d.getDate()))
    s = replaceAll(s, '%d', d.getDate())
    s = replaceAll(s, '%DD', toDayName(d.getDay(), true))
    s = replaceAll(s, '%D', toDayName(d.getDay(), false))
    // Months
    s = replaceAll(s, '%mm', pad(d.getMonth() + 1))
    s = replaceAll(s, '%m', d.getMonth() + 1)
    s = replaceAll(s, '%MM', toMonthName(d.getMonth(), true))
    s = replaceAll(s, '%M', toMonthName(d.getMonth(), false))
    // Years
    s = replaceAll(s, '%yyyy', d.getFullYear())
    s = replaceAll(s, '%yy', d.getFullYear().toString().substring(2))
    // Minutes
    s = replaceAll(s, '%nn', pad(d.getMinutes()))
    s = replaceAll(s, '%n', d.getMinutes())
    // Hours
    s = replaceAll(s, '%hh', pad(d.getHours() % 12 || 12))
    s = replaceAll(s, '%h', d.getHours() % 12 || 12)
    s = replaceAll(s, '%HH', pad(d.getHours()))
    s = replaceAll(s, '%H', d.getHours())
    // Seconds
    s = replaceAll(s, '%ss', pad(d.getSeconds()))
    s = replaceAll(s, '%s', d.getSeconds())
