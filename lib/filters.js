...
filters.ompath = function(input, permalinks) {
    var dot = input.lastIndexOf(".");
    var ext = input.substring(dot+1);
    if (ext === "om" || ext === "omelet" ||
        ext === "md" || ext === "markdown") {
        var inputNoExt = input.substring(0,input.lastIndexOf("."));
        var out = inputNoExt;
        if (permalinks && input.lastIndexOf("/index")!==input.length-6) {
