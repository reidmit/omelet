...
Lexer.prototype = {
    ...
    mode: function() {
        if (this.c === ':') {
            ...
            // Find the indent level of the mode name (e.g. :raw)
            // by working backwards to the last newline
            var iBack = this.index,
                d = this.input[iBack],
                modeNameIndentLevel = 0
            while (iBack > 0 && d !== '\n') {
                if (d === ' ' || d === '\t') {
                    modeNameIndentLevel++
                }
            ...
            // as plain text
            var modeIndentLevel = modeNameIndentLevel + 1

            function peekNextLine(currIndex, input) {
                var i = currIndex + 1,
                    d = input[i],
                    line = ''

                while (i < input.length && d !== '\n') {
                    line += d
                    i++
                    d = input[i]
                }
            ...
            while (this.index < this.len) {
                var line = peekNextLine(this.index, this.input)
                if (line.length === 0 || /^[ \t]+$/.test(line)) {
                    this.consume(line.length + 1)
                    this.token('\n', 'raw')
                } else if ((new RegExp('^[ \t]{' + modeIndentLevel + ',}').test(line))) {
                    if (!seenFirstLine) {
                        indentToRemove = line.search(/[^ \t]/)
                        seenFirstLine = true
                    }
                    line = line.substring(indentToRemove)
                    this.consume(line.length + indentToRemove + 1)
                    this.token(line, 'raw')
                } else {
                    break;
                }
