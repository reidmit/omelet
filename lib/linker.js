...
Linker.prototype = {
    ...
    visit: function(node) {
        ...
        var ast, localDefs, importsAsDefNodes, tagDefsAsDict, importNode
        switch (node.kind) {
            case 'Extend':
                var newAST = this.fileToAst(node.file.value)
                localDefs = this.astToDictionaryNode(this.ast).entries
                importsAsDefNodes = []
                for (importNode of this.ast.imports) {
                    if (importNode.style === 'file' || importNode.style === 'directory') {
                        importsAsDefNodes.push(this.importNodeToDefinitionNode(importNode))
                    } else if (importNode.style === 'tags') {
                        tagDefsAsDict = this.astToDictionaryNode(this.fileToAst(importNode.path.value), true)
