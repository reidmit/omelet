    ...
    function writeFiles(fileInfo, context, prettyPermalinks) {
        ...
        for (var i=0; i<fileInfo.length; i++) {
            ...
            if (!file.isHidden) {
                ...
                if (prettyPermalinks &&
                    getFileExtensionFor(fullPath).length===0 &&
                    fullPath.lastIndexOf("/index")!==fullPath.length-6) {
                    pathWithoutFile = fullPath;
                }
                ...
                rd.renderString(file.contents, file.parser, context, file.filePath, prettyPermalinks);
            }
        }
    }
