...
parsers.omelet = function() {
    ...
    var expr = lazy('an Omelet expression', function() {
                    return alt(RAW,comment,INCLUDE,IF_STATEMENT,FOR_LOOP,tag,parenthetical,ASSIGNMENT,MACRO_DEFINITION,atom);
                });

    // var FILTER_PARAM  = _p("NUMBER | quotedstring | IDENT");
    ...
    var MACRO_ARGUMENT = alt(NUMBER, BOOLEAN, quotedstring, IDENT);

    // var INTERPOLATION = _p("LBRACE (IDENT_COMPLEX | IDENT) MACRO_ARGUMENT* FILTER* RBRACE")
    ...
    var INCLUDE = seq(LPAREN, string("include"), _, regex(/[^\s\)]+/), _, RPAREN)
    ...
    var IMPORT = seq(string("import"), _, regex(/[^\s\)]+/), _, (string("as"), _, IDENT).atMost(1))
                    .toNode(nodes.Import, function(start,value,end) {
                        return {
                            file: value[2],
                            alias: value[4].length===0 ? undefined : value[4][2]
                        }
                    });
